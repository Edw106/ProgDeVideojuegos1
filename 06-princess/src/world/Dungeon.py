...
class Dungeon:
    def __init__(
        ...
        self.current_room = Room(self.player, self.on_game_over, not self.chest_opened)
    ...
    def begin_shifting(self, shift_x: float, shift_y: float) -> None:
        ...
        # Determine entry direction for the new room based on camera shift
        ...
        import random
        # 33% chance to encounter Boss if you have the bow
        if self.player.has_bow and not getattr(self, "boss_defeated", False) and random.randint(1, 3) == 1:
            from src.world.BossRoom import BossRoom
            self.next_room = BossRoom(self.player, self.on_game_over, entry_direction=entry_direction)
        else:
            self.next_room = Room(self.player, self.on_game_over, not self.chest_opened)
    ...
    def update(self, dt: float) -> None:
        ...
        if type(self.current_room).__name__ == "BossRoom":
            if getattr(self.current_room, "boss_defeated_handled", False):
