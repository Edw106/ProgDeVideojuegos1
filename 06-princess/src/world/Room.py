...
class Room:
    def __init__(
        ...
        can_spawn_chest: bool = False,
    ) -> None:
        # Reference to player for collisions, etc.
        self.player = player
        self.on_game_over = on_game_over
        self.can_spawn_chest = can_spawn_chest
        ...
        for projectile in list(self.projectiles):
            ...
            for entity in self.entities:
                ...
                if not entity.dead and projectile.collides(entity):
                    is_arrow = projectile.obj.__class__.__name__ == "Arrow"
                    
                    if hasattr(entity, "sword_immune"):
                        # It's a boss
                        entity.damage(1, is_arrow=is_arrow)
                    else:
                        entity.damage(1)
    ...
    def open_adjacent_chest(self, player: TypeVar("Player")) -> None:
        ...
        for obj in self.objects:
            if obj.type != "chest" or obj.state == "open":
                continue

            obj_col = int((obj.x + obj.width / 2) // settings.TILE_SIZE)
            obj_row = int((obj.y + obj.height / 2) // settings.TILE_SIZE)

            # Less strict: allow the player to be roughly under the chest or slightly overlapping
            adjacent = (abs(obj_col - player_col) <= 1 and 0 <= player_row - obj_row <= 1)

            if adjacent:
                obj.state = "open"
                player.change_state("chest-reward", chest=obj)
                return
    ...
    def _generate_objects(self) -> None:
        ...
        if self.can_spawn_chest and random.randint(1, 3) == 1:
            chest = GameObject(
